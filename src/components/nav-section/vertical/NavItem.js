...
export function NavItemRoot({ item, isCollapse, open = false, active, onOpen }) {
  const { title, path, icon, info, children, isComingSoon } = item;
  ...
  if (isComingSoon) {
    return (
      <ListItemStyle activeRoot={active} isComingSoon>
        {renderContent}
        <Chip label="Coming Soon" size="small" className="comingSoon" />
      </ListItemStyle>
    );
  }
